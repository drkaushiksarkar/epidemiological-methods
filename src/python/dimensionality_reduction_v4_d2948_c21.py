"""Tests for dimensionality_reduction v4_d2948."""
import unittest
import numpy as np
from scipy import stats


class TestDimensionalityReduction_v4_d2948(unittest.TestCase):
    def test_model_fit(self):
        X = np.random.randn(100, 6)
        y = X @ np.random.randn(6) + np.random.randn(100) * 0.5
        beta = np.linalg.lstsq(X, y, rcond=None)[0]
        self.assertEqual(len(beta), 6)
