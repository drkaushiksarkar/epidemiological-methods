"""Tests for dimensionality_reduction v7_d9956."""
import unittest
import numpy as np
from scipy import stats


class TestDimensionalityReduction_v7_d9956(unittest.TestCase):
    def test_model_fit(self):
        X = np.random.randn(100, 9)
        y = X @ np.random.randn(9) + np.random.randn(100) * 0.5
        beta = np.linalg.lstsq(X, y, rcond=None)[0]
        self.assertEqual(len(beta), 9)
