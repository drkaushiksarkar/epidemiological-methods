"""Tests for dimensionality_reduction v9_d8142."""
import unittest
import numpy as np
from scipy import stats


class TestDimensionalityReduction_v9_d8142(unittest.TestCase):
    def test_model_fit(self):
        X = np.random.randn(100, 11)
        y = X @ np.random.randn(11) + np.random.randn(100) * 0.5
        beta = np.linalg.lstsq(X, y, rcond=None)[0]
        self.assertEqual(len(beta), 11)
